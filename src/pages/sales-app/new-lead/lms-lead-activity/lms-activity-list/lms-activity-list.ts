...
export class LmsActivityListPage {
  ...
  constructor(public navCtrl: NavController, public modalCtrl : ModalController, public navParams: NavParams, public viewCtrl: ViewController,private menu: MenuController) {
    ...
    this.converted=this.navParams.get('converted');
    console.log(this.converted,"this is converted data")
  ...
  statusModal(type) 
  {
    let modal = this.modalCtrl.create(ExpenseStatusModalPage,{'from':type,'id':this.checkin_data.id, 'dr_type':this.checkin_data.dr_type,'converted':this.converted});
    modal.present();
  }
