...
export class PopGiftOutgoingPage {
  ...
  getPopList() {
    ...
    if (this.navParams.get('dr_id')) {
      this.serve.addData({ 'dr_id': this.dr_id,'type':this.Type }, 'AppPopGift/outgoingHistory').then((response) => {
      ...
      this.serve.addData({ 'pop_gift_id': this.pop_gift_id,'type':this.Type  }, 'AppPopGift/outgoingHistory').then((response) => {
