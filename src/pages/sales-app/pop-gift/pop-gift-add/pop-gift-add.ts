...
export class PopGiftAddPage {
  ...
  constructor(
    ...
    ionViewDidEnter(){
      this.pop_data.networkType = '1'
      this.get_network_list(this.pop_data.networkType);

      console.log(this.pop_data.networkType,"this is network type")
    }
    ionViewDidLoad() 
    {
    }
    ...
    getPartyPoint() {
      ...
      if (index != -1) {
        this.marketing_points = parseFloat(this.network_list[index]['marketing_points']);
      }
      console.log(this.marketing_points,"marketing Points")
    }

    getPopList()
    {
      this.serve.addData({"Mode": "My","list":"item_list"},'AppPopGift/executivePopGift').then((response)=>
      {
  ...
  getValue(qty) {
    ...
    this.totalPoints = parseFloat(qty) * parseFloat(this.eligible_point);
    this.items.gift_value = this.totalPoints;
  }
    stockValue(id)
    {
      ...
      if (index != -1) {
        ...
        this.eligible_point = parseFloat(this.pop_list[index]['eligible_point'])
    ...
    addToList()
    {
      if (this.pop_data.dr_id != '' && this.totalPoints > this.marketing_points) {
        this.serve.errorToast("Insufficient CP Marketing Points");
        return;
      }
      this.marketing_points = this.marketing_points - this.totalPoints;
      let secondary_index = this.pop_list.findIndex(row=>row.pop_id ==this.pop_data.pop_gift_id.pop_id)
      this.pop_list[secondary_index].qty = parseInt(this.pop_list[secondary_index].qty) - parseInt(this.pop_data.qty)
      let index = this.items.findIndex(row=>row.pop_id ==this.pop_data.pop_gift_id.pop_id)
      
      if(index==-1)
      {
        this.items.push({'item_name':this.pop_data.item_name,  'qty':this.pop_data.qty, 'pop_id' : this.pop_data.pop_gift_id.pop_id, 'gift_value': parseFloat(this.items.gift_value) })
    }
    else
    { 
      this.items[index].qty = parseInt(this.items[index].qty) + parseInt(this.pop_data.qty);
      this.items[index]['gift_value'] += parseFloat(this.items.gift_value);
    }
  ...
  delete_from_list(index){
    this.marketing_points += parseInt(this.items[index]['gift_value']);
