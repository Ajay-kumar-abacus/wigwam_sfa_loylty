...
export class MainDistributorListPage {
  ...
  constructor(
    ...
    this.ModuleName = this.navParams.get('module_name')
    this.currentDate = moment().format("MMM YY");

    if(this.navParams.get('from') =='dashboardPage'){
          this.active_tab='Inactive'
    }
  }
  ionViewWillEnter() {
    ...
    this.storage.get('team_count').then((team_count) => {
      ...
      console.log(this.teamCount, "this is team count");
    });
  ...
  addretailer() {
    this.navCtrl.push(AddRetailerPage, { 'type': this.DrType, 'moduleName': this.ModuleName })
  }
  direct_list: any
  team_list: any
  open() {
  ...
  get_distributor_list(id: any = '') {
    ...
    this.serve.addData({ 
        'Type': this.DrType, 
        'active_tab': (this.DrType == 1 || this.DrType == 7) ? this.active_tab : '', 
        'Mode': this.target_Type, 
        'Master_Search': this.search.name, 
        'User_id': id, 
        'limit': this.limit, 
        'start': this.start 
    }, 'AppCustomerNetwork/distributorLists').then((result) => {
        if (result['statusCode'] == 200) {
            this.distributor_list = result['result'].map(item => this.ensureStringFields(item)); //Apply the helper function here
...
ensureStringFields(item: any): any {
  ...
  loadData(infiniteScroll) {
    this.start = this.distributor_list.length
    this.serve.addData({ 'Type': this.DrType, 'Mode': this.target_Type, 'Master_Search': this.search.name, 'active_tab': (this.DrType == 1 || this.DrType == 7) ? this.active_tab : '', 'User_id': this.asmId, 'limit': this.limit, 'start': this.start }, 'AppCustomerNetwork/distributorLists').then(result => {
      if (result['result'] == '') {
        this.flag = 1;
      }
      else {
  ...
  distributor_detail(dr_id) {
    this.navCtrl.push(LeadsDetailPage, { 'dr_id': dr_id, 'type': 'Dr', 'dr_type': this.DrType, "Mode": this.target_Type })
  }
