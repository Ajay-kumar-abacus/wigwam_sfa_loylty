...
@IonicPage()
@Component({
  selector: 'page-wallet-influencer-wallet',
  templateUrl: 'wallet-influencer-wallet.html',
})
export class WalletInfluencerWalletPage {
  ...
  getLedger(bonus)
  {
    ...
    this.filter.id=this.navParams.get('dr_id')
    this.filter.limit=10;
    this.filter.start=0;
    this.filter.ledger = bonus;
    this.service.addData({'filter' : this.filter},'AppCustomerNetwork/influencerLedgerDr').then(result =>
      {
