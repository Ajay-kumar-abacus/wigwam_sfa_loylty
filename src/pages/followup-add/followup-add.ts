...
import { SiteProjectDetailPage } from '../site-project/site-project-detail/site-project-detail';
...
export class FollowupAddPage {
  ...
  constructor(
    ...
    this.vertualLead_id = this.navParams.get('vertualLead_id')
    this.userId = this.navParams.get('user_id')
    this.followup_data.ExistingCustomer='Non Existing Customer'
  ...
  ionViewDidEnter() {
    this.getNetworkType()
    console.log(this.navParams.get('vertualLead_id'))
    if (this.navParams.get('checkin_id') || this.navParams.get('vertualLead_id')) {
      this.disableSelect = true;
      ...
      this.followup_data.converted = this.navParams.get('converted');
      this.followup_data.dr_type_name = this.navParams.get('dr_type_name');
      this.followup_data.mobile = this.navParams.get('mobile');
  ...
  getDrList(type) {
    ...
    this.followup_data.vertualLead_id = this.vertualLead_id;
    if (this.Page_From == 'checkin_page') {
  ...
  addFollowup() {
    ...
    if (!this.disableSelect) {
      ...
      this.followup_data.mobile = this.followup_data.dr_id.mobile;
      this.followup_data.dr_name = this.followup_data.dr_id.display_name;
    }
    console.log(this.followup_data, "this is followup data")
    this.serve.addData(this.followup_data, 'AppFollowup/addFollowup').then((result) => {
      ...
      if (result['statusCode'] == 200) {
        ...
        if (this.vertualLead_id) {
          this.serve.addData({ 'lead_status': 'Inprocess', 'dr_id': this.followup_data.id, 'type': this.followup_data.dr_type, 'converted': this.followup_data.converted }, 'AppEnquiry/enquiryStageChange').then((result) => {
            if (result['statusCode'] == 200) {
              console.log(result, "this is result")
              let dr_id = result['dr_id'];
              let site_id = result['site_id'];
              ...
              if (dr_type == 15) {
                this.navCtrl.push(SiteProjectDetailPage, { 'id': site_id, 'actionType': 'My' })
