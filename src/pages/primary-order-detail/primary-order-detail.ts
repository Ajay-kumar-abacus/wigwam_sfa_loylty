...
export class PrimaryOrderDetailPage {
    ...
    constructor(public iab:InAppBrowser, public Device:Device, public navCtrl: NavController, private fileOpener: FileOpener, public events: Events,
    ...
    getOrderDetail(order_id) {
        ...
        this.service.addData({ "Id": order_id ,'order_status': this.order_status}, "AppOrder/primaryOrdersDetail").then((result) => {
    ...
    delete_Image(index, id) {
        this.service.addData({ 'id': id }, 'AppOrder/deleteOrdeImage').then((result) => {
            if (result['statusCode'] == 200) {
                this.userDetail.order_images.splice(index, 1);
                this.service.successToast(result['statusMsg'])
                this.getOrderDetail(this.order_id);
            } else {
                this.service.errorToast(result['statusMsg'])
    ...
    ADD_Item(id) {
        ...
        this.navCtrl.push(PrimaryAddItemPage, { "order_item": this.orderitem,'order_status':this.order_status, "order_data": this.userDetail, 'order_id': this.order_id });
    }
    collObject: any = {}
    collapse(index) {
