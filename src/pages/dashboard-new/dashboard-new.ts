...
export class DashboardNewPage {
  ...
  brandWiseReport: any = []
  enquiryReport: any = {}
  PrimaryCustomerReport: any = {}
  ActivityDetail: any = []
  stockCustomerReport: any = {}
  SecondaryCustomerReport: any = {}
  prospectcpReport: any = {}
  Count_values: any=[];
  ...
  constructor(public navCtrl: NavController, public service: MyserviceProvider, public navParams: NavParams, public popoverCtrl: PopoverController) {
    ...
    // }
    // else {
    //   (navParams.get('user_data') != undefined) ? (this.user_data = navParams.get('user_data'), this.assigned_orderType = this.user_data.order_type.toLowerCase()) : null;
    //    ((this.assigned_orderType == 'both') || (this.assigned_orderType == 'primary')) ? (this.primaryDashboard = true, this.secondaryDashboard = false) : (this.primaryDashboard = false, this.secondaryDashboard = true);
  ...
  ionViewWillEnter() {
    ...
    this.getCustomerCountReport()
    this.getLeadReport()
    this.getProspectCpReport()
  ...
  getCustomerReport() {
    let apiName = ''
    if (this.primaryDashboard) {
      apiName = 'AppDashboard/orderTypeSales'
    } 
    this.service.addData({}, apiName).then((response) => {
      if (response['statusCode'] == 200) {
        // this.PrimaryCustomerReport = response['result']
        this.PrimaryCustomerReport = response['result']['primary']
        this.SecondaryCustomerReport = response['result']['secondary']
        this.stockCustomerReport = response['result']['stock']
  ...
  getCustomerCountReport() {
    let apiName = '';
    if (this.primaryDashboard) {
      apiName = 'AppDashboard/partywise_dr_count';
    }
  
    this.service.addData({}, apiName).then((response) => {
      if (response['statusCode'] == 200) {
        this.Customer_count = response['result'];
        this.labels = this.Customer_count.module;
        console.log(this.labels, "Labels for Customer Count Chart");
        this.Count_values = this.Customer_count.module_count;
  
        let customerBarChart: any = {
          type: 'bar',
          backgroundColor: '#fff',
          ...
          plotarea: {
            margin: '30px 15px 40px 15px', // Increased margin for better spacing
            backgroundColor: 'transparent',
            borderRadius: '10px',
            borderWidth: '0px',
          },
          ...
          scaleX: {
            labels: this.labels, // Ensure labels are correct
            item: {
              fontSize: '12px', // Increased font size for better readability
              fontColor: '#000', // Set label color for better contrast
              angle: -80,
             
            },
            visible: true, // Ensure labels are visible
            tick: {
              lineColor: '#000', // Tick color for better visibility
            },
          },
        ...
        customerBarChart.gui = { contextMenu: { visible: false } };
        zingchart.render({
          id: 'customerBarChart',
          data: customerBarChart,
          height: 350,
        });
      }
    }, (error) => {
  ...
  getLeadReport() {
    let apiName = 'AppDashboard/lead_count_as_pr_priority';
    
    this.service.addData({}, apiName).then((response) => {
      if (response['statusCode'] == 200) {
        this.leadReport = response['result'];
        console.log(this.leadReport);
        ...
        let enquiryPieChart: any = {
          ...
          series: [
            {
              text: 'Win-F',
              values: [this.leadReport.win_f_count],
              backgroundColor: '#67a21e',
              lineColor: '#67a21e',
              lineWidth: '1px',
              marker: {
            ...
            {
              text: 'Win-C',
              values: [this.leadReport.win_c_count],
              backgroundColor: '#039639',
              lineColor: '#039639',
              lineWidth: '1px',
              marker: {
                backgroundColor: '#039639',
            ...
            {
              text: 'Lost',
              values: [this.leadReport.lost_count],
              backgroundColor: '#ff4441',
              lineColor: '#ff4441',
              lineWidth: '1px',
              marker: {
            ...
            {
              text: 'Open Hot',
              values: [this.leadReport.open_hot_count],
            ...
            {
              text: 'Open Warm',
              values: [this.leadReport.open_warm_count],
  ...
  getProspectCpReport() {
    let apiName = 'AppDashboard/prosectCp__monthly_detail';
    
    this.service.addData({}, apiName).then((response) => {
      if (response['statusCode'] == 200) {
        this.prospectcpReport = response['result'];
        ...
        let ProspectCpPieChart: any = {
          ...
          series: [
            ...
            {
              text: 'Open',
              values: [this.prospectcpReport.total_open_prospectcp_count],
            ...
            },
            {
              ...
              values: [this.prospectcpReport.converted_to_subdealer_count],
              backgroundColor: '#ff4441',
              lineColor: '#ff4441',
              lineWidth: '1px',
              marker: {
            ...
            {
              text: 'ConvertedCP',
              values: [this.prospectcpReport.converted_to_cp_count],
              backgroundColor: '#7bff00',
              lineColor: '#7bff00',
              lineWidth: '1px',
              marker: {
        ...
        zingchart.render({ id: 'ProspectCpPieChart', data: ProspectCpPieChart, height: 250 });
      } else {
        // Handle errors
      }
    }, (error) => {
  ...
  getActivityReport() {
    let apiName = ''
    if (this.primaryDashboard) {
      apiName = 'AppDashboard/sales_activity'
    } 
    this.service.addData({'date_range':this.selectedDateRange}, apiName).then((response) => {
      if (response['statusCode'] == 200) {
        // this.PrimaryCustomerReport = response['result']
      ...
      if (response['statusCode'] == 200) {
        ...
        let DigitalPieChart: any = {
          ...
          series: [
            {
              text: 'Qualified',
              values: [this.DigitalReport.qualified_count],
            ...
            {
              text: 'InProcess',
              values: [this.DigitalReport.inprocess_count],
              backgroundColor: '#ffb300',
              lineColor: '#ffb300',
              lineWidth: '1px',
              marker: {
            ...
            {
              text: 'Win',
              values: [this.DigitalReport.win_count],
              backgroundColor: '#67a21e',
              lineColor: '#67a21e',
              lineWidth: '1px',
              marker: {
            ...
            {
              text: 'Lost',
              values: [this.DigitalReport.lost_count],
              backgroundColor: '#ff4441',
              lineColor: '#ff4441',
              lineWidth: '1px',
              marker: {
